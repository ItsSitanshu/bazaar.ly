...
export default function SettingsPage({ params }: { params: Promise<{ store: string }> }) {
  ...
  return (
    ...
    <div className="flex h-screen w-screen justify-start items-center">
      <DashboardSideBar shopName={store} currentPage="Settings" logoUrl={_store.logo_url}/>
      <div className="flex flex-col w-full h-full justify-start items-center">
        ...
        <div className="flex flex-row justify-between items-end w-full h-16 pl-[3em] pr-[3em]">
          <div className='flex flex-row justify-center items-center h-full'><div className="relative">
            <div className="flex justify-center items-center w-[7em] h-[7em] p-1 rounded-full bg-trasparent border-2 border-white">
            {_store.logo_url ? (
              <Image src={_store.logo_url} width={32} height={32} alt='?' className='w-[7em] h-[7em]'/>
            ) : (
              <Image src={questionMark} alt="File icon" width={32} height={32} className='w-16 h-16'/>
            )}
            </div>
          </div>
          <div className="flex flex-col pl-3 pt-2">
            <h1 className='font-work font-bold text-2xl text-white'>{_store.store_name}</h1>
            <h1 className='font-work font-light text-[0.6em] text-white'>{_store.subdomain}.bazaar.ly</h1>
          </div></div>
